...
def BA(iter, states, velocities, imu_meas, landmarks, landmarks_xyz, ii, time_idx, intrinsics, confidences, Sigma, V, lamda_init, poses_gt_eci, initialize=False, printi=False):
	...
	bsz = states.shape[0]
	res_dist, Jd = residual_reldist(states, neighbor_poses, poses_gt_eci, jacobian=True)
	landmark_est, Jg = landmark_project(states, landmarks_xyz, intrinsics, ii, jacobian=True)
	if torch.cuda.is_available():
		r_pred, pose_pred, vel_pred, Ji, Ji_1, Jf, Hq, qgrad = predict_gpu(states, imu_meas, time_idx, quat_coeff, vel_coeff, jacobian=True, initialize=initialize) 
	else:
	...
	dim = 9
	Jg = Jg.reshape(bsz, -1, dim_base)[:, :, :dim].reshape(-1, 2, dim)
	Jd = Jd.reshape(bsz, -1, dim_base)[:, :, :dim].reshape(-1, 2, dim)
	
	JgTwJg = torch.bmm((Jg*wts_obs).transpose(1,2), Jg).reshape(bsz, -1, dim, dim)
	JdTwJd = torch.bmm((Jd*wts_obs).transpose(1,2), Jd).reshape(bsz, -1, dim, dim)
	...
	JgTwJg = safe_scatter_add_vec(JgTwJg, kk_t, n).view(bsz, n, dim, dim)
	JgTwJg = torch.block_diag(*JgTwJg[0].unbind(dim=0)).unsqueeze(0)
